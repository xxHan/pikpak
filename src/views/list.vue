  ...
  <div class="list-page list-page-files">
    <div class="header">
      ...
      <div class="action">
        <n-space>
          ...
          <n-button @click="showUserMenu = true"> 添加自定义菜单 </n-button>
    ...
    <n-modal v-model:show="showAddUrl">
      <n-card style="width: 600px" title="添加链接或新建文件夹">
        ...
        <n-alert :show-icon="false" closable title="添加说明">
          <div>
            1.支持Magnet链接(magnet:?xt=urn)，Magent链接只能默认保存到My Pack
          </div>
          <div>
    ...
    <n-modal v-model:show="showImage">
      ...
      >
        ...
        <div
          style="width: 100%; height: calc(100vh - 80px); text-align: center"
        >
          <img
    ...
    <n-modal v-model:show="showUserMenu">
      <n-card style="width: 600px" title="自定义菜单">
        <template #header>
          自定义菜单
          <a href="https://www.tjsky.net/?p=220#i-8" target="_blank">
            <n-icon style="vertical-align: middle" size="20" color="#d03050"
              ><zoom-question></zoom-question
            ></n-icon>
          </a>
        </template>
        <template #header-extra>
          ...
          <n-form-item label="自定义菜单列表：">
            <n-space>
              <template v-for="(item, key) in userMenu" :key="key">
                <n-tag :closable="true" @close="removeUserMenu(key)">{{
                  item.name
                }}</n-tag>
              </template>
            </n-space>
          ...
          <n-form-item label="自定义菜单可插入：">
            <n-space>
              <template v-for="(item, k) in menuTextList" :key="k">
                <n-button
                  @click="newMenu.content = newMenu.content + '{{' + k + '}}'"
                  >{{ item }}</n-button
                >
              </template>
            </n-space>
        ...
        <n-form label-width="40px" label-align="left" label-placement="left">
          <template v-for="item in fileInfo?.medias" :key="item.media_id">
            <n-form-item :label="item.media_name">
              <n-input-group>
                ...
                <n-button type="primary" @click="copy(item.link.url)"
                  >复制</n-button
                >
              </n-input-group>
            </n-form-item>
          ...
          <n-form-item label="链接">
            <n-input-group>
              ...
              <n-button type="primary" @click="copy(fileInfo.web_content_link)"
                >复制</n-button
              >
            </n-input-group>
          </n-form-item>
...
const columns = ref<DataTableColumns>([
  ...
  },
  {
    ...
    align: "right",
    render: (row) =>
      Number(row.size) > 0 ? byteConvert(Number(row.size)) : "",
    className: "size",
    width: 100,
  },
  {
                              ...
                              return template.replace(
                                ...
                                (match, key) => {
                                  ...
                                  if (
                                    ...
                                    data =
                                      res.data.medias[0]?.link?.url || data;
                                  }
                                  return data;
                                }
                              );
...
const initPage = () => {
  moveFiles.value = JSON.parse(
    window.localStorage.getItem("pikpakMoveFiles") || "[]"
  );
  copyFiles.value = JSON.parse(
    window.localStorage.getItem("pikpakCopyFiles") || "[]"
  );
  userMenu.value = JSON.parse(
    window.localStorage.getItem("pikpakUserMenu") || "[]"
  );
  filesList.value = [];
  ...
  urlList.forEach((url: string) => {
    if (url) {
      ...
      http
        ...
        .then((res: any) => {
          if (
            res.data.upload_type === "UPLOAD_TYPE_UNKNOWN" ||
            url.indexOf("PikPak://") === -1
          ) {
  ...
  nextTick(() => {
    ...
    clipboard.on("error", (e) => {
      window.$message.error(
        "复制失败，您可以F12打开控制台手动复制，或手动复制弹窗输入框"
      );
...
const namePost = () => {
  http
    .patch(
      "https://api-drive.mypikpak.com/drive/v1/files/" + newName.value?.id,
      {
    ...
    axios
      ...
      .then((res: any) => {
        ...
        copy(
          sharePikPakUrl.value +
            (sharePikPakPassword.value
              ? " 提取密码  " + sharePikPakPassword.value
              : "")
        );
      })
      .catch((res) => {
